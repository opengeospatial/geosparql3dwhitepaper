= GeoSPARQL 3d White Paper
:doctype: white-paper
:language: en
:status: draft
:committee: technical
:workgroup: Geosemantics DWG
:draft: 1.0
:external-id: http://www.opengis.net/doc/WP/geosparql3d
:comment: ### Internal reference number; mandatory
:docnumber: YY-999
:relaton-data-source: bibliography.bib
:received-date: 2029-03-30
:comment: ### Date on which the standard was approved by the issuing authority; mandatory
:issued-date: 2029-03-30
:comment: ### Date on which the standard was published; mandatory
:published-date: 2029-03-30
:fullname: Editor One
:fullname_2: Editor Two
:role: editor
:imagesdir: figures
:keywords: OGC, GeoSPARQL, 3D
:submitting-organizations: Organization one; Organization two; Organization three
:mn-document-class: ogc
:mn-output-extensions: html,pdf
:pdf-uri: ./document.pdf
:edition: 1.0
:toc:
:toc-placement: left
:toclevels: 4
:numbered:
:sectanchors:

:sectnums!:

include::sections/01-front_material.adoc[]

include::sections/02-preamble.adoc[]

:sectnums:

include::sections/03-introduction.adoc[]

include::sections/04-motivation.adoc[]

include::sections/05-curcapabilities.adoc[]

include::sections/06-related-work.adoc[]

include::sections/07-geosparql3D.adoc[]

include::sections/08-benefits.adoc[]

:sectnums!:

//use annexes as needed
include::sections/90-annex-a.adoc[]

include::sections/91-annex-b.adoc[]

include::sections/92-annex-c.adoc[]

////
Revision History should be the last annex before the Bibliography
Bibliography should be the last annex
////
include::sections/98-annex-history.adoc[]

include::sections/99-annex-bibliography.adoc[]
