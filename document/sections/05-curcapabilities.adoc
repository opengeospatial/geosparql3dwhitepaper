...
Realizing the motivation described above requires technical and semantic support for the standardized representation, integration, and querying of 3D spatial information. GeoSPARQL could provide such a foundation. First it is necessary to understand the current capabilities of the standard. This provides the basis for considering where additional capbilities or extensions may be needed. 
...
.Difference between 2D, 2D with height attribute 2.5D and 3D geometry
image::../figures/2d-2d--2andahalfd-3d-difference.png[align="center"]


[cols="3,3,3,3,3"] 
|=== 
|Literal Type | Z-Coordinate Supported |Z-Coordinate Supported + height attribute | 2.5D | 3D 
|WKT Literal  | Yes | Yes | Yes | Yes
|GML Literal  | Yes | Yes | Yes | Only with extension Schema
|KML Literal  | Yes | Yes | Yes | As import from COLLADA
|GeoJSON Literal  | Yes | Yes | Yes | Yes
|DGGS Literal  |  |  | |
|=== 
...
See Annex A and Annex B (Text to write)
