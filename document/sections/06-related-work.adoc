...
Various vocabularies, standards, and domain-specific approaches provide concepts for describing 3D geometries, topology, spatial relationships, and their semantics. This section reviews these related developments to understand the different ways in which 3D spatial information is currently modeled and to identify concepts and approaches that may complement or inform the further development of 3D in GeoSPARQL.

A substantial body of work shows that **there is no single universal 3D web ontology**, but several reusable, often domain‑ or format‑specific vocabularies to describe geometry, topology, scenes, and domain semantics. Two principal flavours of ontologies can be identified in existing literature:i) those meant for the generic representation of 3D objects and shapes, and ii) a growing body of domain-specific ontologies <<flotynski2017ontology>>.

This section starts by providing a brief overview of these two flavours of ontologies. It then dives into domain-specific initiatives, considering Cultural Heritage and Geo-spatial in particular. This section closes by enumerating various encoding formats relevant for 3D.


=== 3D Exchange formats 
A new GeoSPARQL standard should be compatible with existing approaches to encode 3D data.
Semantic technologies should enable users to encode important metadata in the knowledge graph, while retaining the possibility to use already well-known 3D geometry formats either as String literals or file references.
...
==== Standard for the Exchange of Product model data (STEP) and OntoSTEP
https://www.steptools.com/stds/step/[STEP (Standard for the Exchange of Product model data)] is an ISO standard for representing and exchanging product and engineering information, including CAD models. OntoSTEP translates STEP concepts into an ontology-based representation to support semantic integration and reasoning. OntoSTEP is a research-oriented approach that translates STEP/EXPRESS schemas and instances into OWL to enable semantic integration, querying, and reasoning. Initially developed at NIST, it has been further investigated through later implementations, but it remains a research approach rather than an established standard or widely adopted 3D ontology. 

- OWL encodings of STEP AP203/AP214/AP242 and BREP/CSG concepts for CAD models <<zhao2024envon>>.  
- Other proposals link geometric models to spatial properties or count non‑manifold features <<FlotynskiWalczak2017>> <<zhao2024envon>>.  
...
Nowadays it is mainly used for 3D printing purposes, which makes is a plausible candidate for support in GeoSPARQL 3D settings of movable objects such as in cultural heritage.
...
The https://paulbourke.net/dataformats/ply/[Polygon File Format] is a format which was designed to store 3D data from 3D scanning environments, which makes it especially widespread in the Cultural Heritage application area.
The format consists of a vertex list and a faces list with optional parameters per vertex.
Like the OBJ format, supporting PLY would imply including support for vertices and faces of a 3D model and their relationships in a knowledge graph.


=== Geometry metadata and linking 
...
=== Generic 3D representation and semantic models 

==== X3D Ontology

Extensible 3D (X3D) Graphics is an open standard to declaratively representing interactive 3D computer graphics for publishing, viewing, printing and archiving 3D models on the Web. X3D was eventually approved by ISO (ISO/IEC 19775-19777) and become an computer graphics industry standard. It was first proposed in 1997 and has since been subject to various updates. More recently it has been developed towards the Semantic Web, in order to gain interoperability and enable intelligent 3D operations and reasoning. Development has been hosted by the Web3D Consortium. 

Currently in its version 4, the X3D Ontology maps the full X3D standard (X3DUOM) into OWL 2, providing classes and properties for nodes, geometry, appearance, animation, and scene graph structure, enabling query and reasoning over X3D scenes <<FlotynskiWalczak2017>> <<georgieva2023transforming>> <<Sikos2017>> <<BrutzmanFlotynski2020>> <<flotynski2020recent>> <<8975906>>.  X3D is regarded as a comprehensive and formally grounded 3D ontology, adding fundamental concepts and roles of the 3D modelling industry not originally covered by X3D.

The X3D Ontology has also been used to transform non-semantic 3D file formats into RDF with languages such as RML or XSLT <<georgieva2023transforming>> <<BrutzmanFlotynski2020>>.
...
X3D is an interesting format to investigate for 3D components which need to be modeled within a GeoSPARQL 3D specification. It is a likely candidate for a literal format and might be integrated using interlinking approaches to the already existing X3D vocabulary.
...
==== VRML / Java3D spatial ontologies  
Application ontologies abstracting VRML & Java3D node types plus a separate spatial ontology for topological, directional and metric relations <<pacheco2018creating>>.

==== Common Shape Ontology
– represents manifold/non‑manifold shapes, point sets, hierarchical model groups, positions, orientations, keyframe animation <<FlotynskiWalczak2017>>.  


=== Domain‑specific 3D ontologies

==== Geospatial and 3D City Model (CityGML)

**CityGML and OntoCityGML**
...
Building-conservation ontology for 3D annotations. The conservation and restoration of historical monuments require the integration of information from many different disciplines and techniques. This study presents a pipeline that combines semantic, spatial, and morphological data into one 3D information model. The approach was tested on Saint Maurice Church in Caromb, France, allowing conservation experts to better visualize, correlate, and retrieve information about materials, damage, and other conservation observations. <<messaoudi2018heritage>>; 

CIDOC CRM–based models for 3D CH objects and provenance. This paper proposes an ontology-based approach for managing and using 3D content without requiring domain experts to have programming or 3D modelling skills. The actual 3D elements can be created by graphic designers and then linked to domain-specific concepts by developers. Ontologies and automated reasoning can subsequently determine which 3D elements are relevant based on the specified domain information, while semantic queries allow the content to be explored and analysed. The focus is therefore on connecting 3D graphics with domain knowledge rather than on creating the 3D models themselves. <<walczak2019inference>> <<amico2106ontological>> <<гончар2021ontological>>. 

**Use Cases**

Use Cases in the Cultural Heritage domain include, but are not limited to, the following main interests:

**Visualization:** The visualization of 3D models for the presentation of such 3D models, for example in the context of a museum. 3D models may be styled with a particular set of textures or modeled with a specific set of colours to highlight certain relevant aspects. The visualization of 3D models is currently standardized by the IIIF 3D working group <<Haynes2023iiif3d>>, which aims to create viewing parameter descriptions that 3D viewers may implement, similar to the specifications of IIIF 2D for images.
...
Cuneiform tablets from ancient Iran provide an interesting research area, since they combine a 3D artifact with textual imprints that are of interest to a variety of research communities including Assyriologsts, Digital Humanists, Computational Linguists and last but not least Computer Scientists.
...
Knowledge graphs provide machine learning approaches <<Stotzner_2023_ICCV>> <<stotzner2023r>> with unique opportunities to connect different kinds of data using a unified data format.
...
The viewer specification includes the visualization of 3D models including lighting positions, a scene graph and annotations in the form of additional 3D objects added to a 3D scene.
A GeoSPARQL 3D standard might need to be compatible with, or include a compatibility statement of, converting 3D objects encoded in GeoSPARQL to the IIIF viewer specification. A prototype implementation might be necessary to comply with this specification.

==== ChgOnto
Grottoes & heritage ontology. This paper focuses on enhancing 3D models of Chinese grottoes by adding semantic information. It develops a knowledge graph that links 3D elements—such as caves, Buddha statues, walls, and decorations—to their properties and spatial relationships, making the 3D models more meaningful, understandable, and useful for cultural heritage documentation and analysis.for grotto components, spatial relations, with GeoSPARQL + CIDOC CRM <<yang2023knowledge>>. 

==== OntPreHer3D
OntPreHer3D is an extension of the CIDOC CRM ontology. It is designed to support the FAIR preservation and documentation of 3D cultural heritage models. The ontology captures relationships between 3D models, their components, and real-world counterparts, while also documenting the reasoning, uncertainties, and decisions behind hypothetical reconstructions. A case study of the Speyer Synagogue demonstrates how differences in modelling processes can affect final 3D models, highlighting the importance of detailed documentation for the long-term preservation, reproducibility, and reuse of digital cultural heritage.  https://peercommunityjournal.org/articles/10.24072/pcjournal.608/[OntPreHer3D: Ontology for Preservation of Cultural Heritage 3D Models]
...
(Identifying faces in procedural geometries is also challenging (toplogical naming problem), so using geometric semantic overlays to annotate potentially procedural geometries makes sense)
...
AEC - Building domain ontology for storeys, spaces, elements + 3D mesh links; used with separate geometry vocabularies <<flotynski2020bot>>. |

===== Linked Building Server 
Over the last years, Building Information Modelling (BIM) has been increasingly adopted in building projects for its benefits to collaboration and associated risk mitigation. Recent work is trying to make this data available over the web (web-based BIM applications). Most notable in making building data web-ready is the linked building data (LBD) initiative. This group has worked on a number of vocabularies and ontologies, thereby starting from a linked data approach (RDF graphs), aiming at representing building data on the web. https://github.com/LBDserver[Linked Building Data Server]

==== ENVON
Robotics / manufacturing ontology that integrates contextual, topologic, geometric info for 3D environments; BFO‑based 3D shape ontology; BREP/CSG distinctions <<zhao2024envon>>. 


**Use of domain specific ontologies** 
Domain-specific 3D ontologies are typically expressed using **RDF/OWL**, defining concepts and relationships that capture the semantics of a particular application domain <<FlotynskiWalczak2017>> <<pittarello2006semantic>> <<walczak2019inference>>.  

Geometry, topology, scene structure, and domain semantics are often represented by separate but complementary ontologies that can be combined at query time for integrated queries, such as BOT with geometry and sensor (SOSA/SSN or SAREF) ontologies in building applications <<zhao2024envon>> <<flotynski2020bot>>.  


==== Location ontologies

===== Relative Location Ontology (Reloc) 
The https://annegoebels.github.io/reloc/[Relative Location Ontology (RELOC)] enables the expression of the spatial relationship between two objects using directional-topological terms. The ontology is designed to be compatible with the natural language-based approach of describing relative locations, enabling the conversion of existing location descriptions into structured spatial graph representations. The ontology provides directional, axis-related properties (e.g., longitudinal axis: front, center, rear) and topological properties (e.g., meet, contained in, intersect). These properties are finally merged into combined properties (e.g., containedInFront) to reflect the spatial information of statements like 'A is in the front part of B'. The application context of the RELOC Ontology is the processing of existing damage documentation of bridges.

===== InfraspatialOT 
The InfraspatialOT ontology, described in the paper: https://www.researchgate.net/publication/394086571_infraspatialOT_An_Ontology_for_the_Representation_of_Spatial_Relationships_in_Road_Infrastructure[infraspatialOT: An Ontology for the Representation of Spatial Relationships in Road Infrastructure], provides a semantic model for representing road infrastructure elements and their spatial relationships to reference elements. It distinguishes between localisation, which describes where an infrastructure element is located along a reference element, and positioning, which captures its relative spatial position, including horizontal and vertical relationships. Positioning relations can further specify whether a distance is involved. This structure enables road infrastructure elements and their spatial relationships to be represented and queried consistently using semantic technologies such as SPARQL.

==== Medicine and Chmistry
===== 3D Microscopy Metadata Standards (3D-MMS)
The 3D Microscopy Metadata Standards (3D-MMS) was developed by the BRAIN Initiative to establish a consistent way of describing 3D microscopy datasets, particularly datasets of brain tissue. Its purpose is to provide enough information for someone who did not generate the data to understand, find, interpret, reuse, and potentially integrate the dataset with other datasets. The 3D-MMS-metadata can be used to describe how the image volume is oriented and scaled, so that a position inside the microscopy dataset has meaningful spatial coordinates.
<<ropelewski_standard_2022>> 

=== 3D Topology

==== Topological Relationships and Their Use
The book chapter https://repository.tudelft.nl/record/uuid:09075b7e-e019-45f1-becc-cf8d7992b63b[Topological Relationships and Their Use] highlights that topological relationships are well established for simple 2D objects, while 3D topological operations remain less developed. It identifies the need to support more complex 3D objects, including solids with holes and tunnels, and suggests reusing existing spatial operators as a starting point for extending topological analysis to 3D.

==== Extension of RCC*-9 to Complex and Three-Dimensional Features and Its Reasoning System
The work described in the paper https://www.mdpi.com/2220-9964/13/1/25[Extension of RCC*-9 to Complex and Three-Dimensional Features and Its Reasoning System], defines semantics of 3D bodies, regions, and lines, which can support the definition and interpretation of 3D spatial relations and operations. It provides a formal topological foundation that can be interesting for GeoSPARQL 3D.

=== Implementations
The related work above primarily address 3D geometry semantics. A complementary aspect is the computational processing of the 3D representations. Implementations of 3D geometry processing provide the algorithms and data structures required to perform operations such as intersection, containment, distance calculation, Boolean operations, and topology analysis. This section  reviews established geometry-processing libraries and systems, including CGAL, SFCGAL, OpenCASCADE, and Topologic, and considers their relevance to the implementation of 3D spatial operations in GeoSPARQL.

==== CGAL &#8594; SFCGAL &#8594; PostGIS
CGAL (Computational Geometry Algorithms Library) provides data structures and algorithms for computational geometry, including operations on geometric objects that can serve as a computational foundation for spatial processing. SFCGAL builds on CGAL to provide additional support for 3D and volumetric geometries and operations, including Boolean operations and geometric predicates. SFCGAL is integrated with PostGIS, a spatial database extension for PostgreSQL, allowing such geometry-processing capabilities to be used within a database environment. Together, these technologies illustrate how 3D geometric representations can be connected to computational geometry algorithms and exposed through a spatial database system, providing a relevant implementation perspective for 3D spatial operations in GeoSPARQL.

==== OpenCASCADE
OpenCASCADE is an open-source software platform for 3D geometric modelling and CAD applications. It provides data structures and algorithms for representing and processing geometric and topological entities, including points, curves, surfaces, solids, and B-Rep models. The platform supports operations such as geometric transformations, Boolean operations, intersections, and shape analysis. Its explicit representation of geometry and topology makes OpenCASCADE relevant for the computational processing of 3D solids and for investigating how B-Rep-based geometry could be supported by a GeoSPARQL 3D implementation.
...
B-Rep and mesh/polyhedron representations differ in how 3D geometry and topology are modelled. B-Rep explicitly represents curved surfaces and topological relationships between vertices, edges, faces, shells, and solids, making it suitable for complex solid models, but it can involve more complex data structures and validation requirements. Mesh and polyhedral representations generally use simpler discrete geometries, often based on triangles, with several possible data structures for representing adjacency and topology, such as half-edge, winged-edge, and indexed face sets. For GeoSPARQL 3D, this distinction is relevant because the underlying representation can affect the precision, robustness, and implementation of spatial operations.
