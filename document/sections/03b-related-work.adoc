...
A substantial body of work shows that **there is no single universal 3D web ontology**, but several reusable, often domain‑ or format‑specific vocabularies to describe geometry, topology, scenes, and domain semantics. Two principle flavours of ontologies can be identified in existing literature: i) those meant for the generic representation of 3D objects and shapes, and ii) a growing body of domain-specific ontologies <<flotynski2017ontology>>.

This section starts by providing a brief overview of these tow flavours of ontologies. It then dives into domain-specific initiaves, considering Cultural Heritage and Geo-spatial in particular. This section closes by enumerating various encoding formats relevant for 3D.


=== Generic representation of 3D objects
...
==== Core 3D shape/geometry ontologies
...
- **Common Shape Ontology** – represents manifold/non‑manifold shapes, point sets, hierarchical model groups, positions, orientations, keyframe animation <<FlotynskiWalczak2017>>.  

- **BREP/CSG‑oriented ontologies & OntoSTEP** – OWL encodings of STEP AP203/AP214/AP242 and BREP/CSG concepts for CAD models <<zhao2024envon>>.  
...
|===
| Domain | Example ontologies / vocabularies | Citations 

| Buildings / AEC | **BOT (Building Topology Ontology)** for storeys, spaces, elements + 3D mesh links; used with separate geometry vocabularies <<flotynski2020bot>>. |
...
| Robotics / manufacturing | **ENVON** integrates contextual, topologic, geometric info for 3D environments; BFO‑based 3D shape ontology; BREP/CSG distinctions <<zhao2024envon>>. |
| Grottoes & heritage | **ChgOnto** for grotto components, spatial relations, with GeoSPARQL + CIDOC CRM <<yang2023knowledge>>. |
| Medicine & Chemistry | **3D Microscopy Metadata Standards (3D-MMS)** | <<ropelewski_standard_2022>> 
| Geo-spatial | **OntoCityGML** | <<CHADZYNSKI2021100106>> |
|===

**Figure 1:** Representative groups of 3D-related web ontologies

==== How these are typically used

- Built on **RDF/OWL** with RDFS/OWL vocabularies for 3D content and domain classes <<FlotynskiWalczak2017>> <<pittarello2006semantic>> <<walczak2019inference>>.  
- Often separate **geometry/topology**, **scene graph**, and **domain** ontologies combined at query time (e.g., BOT + geometry + SOSA/SSN or SAREF in buildings) <<zhao2024envon>> <<flotynski2020bot>>.  
